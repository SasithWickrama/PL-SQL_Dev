CREATE OR REPLACE PROCEDURE CLARITY_ADMIN.PEND_MEGALINE_WO_MAIL IS

    CURSOR C_DETAIL IS     
     SELECT count(*)
     FROM CLARITY_ADMIN.PENDING_MEGALINE_WO_DETAILS;
...
SELECT
          ...
          -----  CASE WHEN SUBSTR(OWNER,1,2) = 'Z_' THEN SUBSTR(OWNER,3) ELSE OWNER END AS OWNER, 
            ...
            MOD_FEATURE_OR_L,
            MOD_FEATURE_OR_G,
            (NEW_CON_L+CREATE_OR_L+CREATE_RECON_L+MOD_LOCATION_L+MOD_LOC_SAME_DP_L+MOD_FEATURE_L+MOD_FEATURE_OR_L) NEW_CONN_SUB_TOTAL_L,
            (NEW_CON_G+CREATE_OR_G+CREATE_RECON_G+MOD_LOCATION_G+MOD_LOC_SAME_DP_G+MOD_FEATURE_G+MOD_FEATURE_OR_G) NEW_CONN_SUB_TOTAL_G,
            REMOVAL_L,
            REMOVAL_G,
            DELETE_OR_L,
            DELETE_OR_G,
          ...
          ----  CASE WHEN OWNER IS NULL THEN 'Z_SUB TOTAL' ELSE OWNER END AS OWNER,
            ...
            COUNT(CASE  WHEN ORDER_TYPE = 'CREATE-RECON'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) <= 7 ) THEN 'X' ELSE NULL END) "CREATE_RECON_L",
            ...
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-LOC SAMEDP'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) <= 7 ) THEN 'X' ELSE NULL END) "MOD_LOC_SAME_DP_L",
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-FEATURE'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) <= 7 ) THEN 'X' ELSE NULL END) "MOD_FEATURE_L",
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-FEATURE-OR'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) <= 7 ) THEN 'X' ELSE NULL END) "MOD_FEATURE_OR_L",
            ...
            COUNT(CASE  WHEN ORDER_TYPE = 'CREATE-RECON'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) > 7 ) THEN 'X' ELSE NULL END) "CREATE_RECON_G",
            ...
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-LOC SAMEDP'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) > 7 ) THEN 'X' ELSE NULL END) "MOD_LOC_SAME_DP_G",
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-FEATURE'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) > 7 ) THEN 'X' ELSE NULL END) "MOD_FEATURE_G",
            COUNT(CASE  WHEN ORDER_TYPE = 'MODIFY-FEATURE-OR'  AND ((TRUNC(SYSDATE) - TRUNC(TO_DATE(ASSIGNED_DATE ,'DD-MON-YY'))) > 7 ) THEN 'X' ELSE NULL END) "MOD_FEATURE_OR_G",
         ...
         CURSOR CLA_PRESENTAGES  IS 
             SELECT
            '<tr bgcolor="#33FF00" ><b><td colspan="3">PERCENTAGE  </td>'||----CCFFFF
            ...
            '<td>'||ROUND(((CREATE_RECON_L  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((CREATE_RECON_G  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_LOCATION_L  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_LOCATION_G  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_LOC_SAME_DP_L  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_LOC_SAME_DP_G  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_FEATURE_L  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_FEATURE_G  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_FEATURE_OR_L  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MOD_FEATURE_OR_G  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND((((NEW_CON_L+CREATE_OR_L+CREATE_RECON_L+MOD_LOCATION_L+MOD_LOC_SAME_DP_L+MOD_FEATURE_L+MOD_FEATURE_OR_L)  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND((((NEW_CON_G+CREATE_OR_G+CREATE_RECON_G+MOD_LOCATION_G+MOD_LOC_SAME_DP_G+MOD_FEATURE_G+MOD_FEATURE_OR_G)  / TOTAL) *100),0) ||'%</td>'||
  ...
  l_name VARCHAR2(80) := 'MEGALINE_Pending_WO_Details.xls';
  l_blob CLOB;
  l_body           VARCHAR2(32700); ---24000
  v_attr_table_html varchar2(24000);  --- 14000
  v_attr_table_1_html varchar2(14000);
  v_get_cal_pres varchar2(3000);
  dest_lob CLOB;
...
dbms_output.put_line('@6') ;
 ...
 dbms_output.put_line('@7') ;
  ...
  v_sms_txt := 'Dear%20All%20MEGALINE%20pending%20WO%20detail%20report%20as%20@%20today%20contain%20' ||v_ren_CNT|| '%20records.%20Please%20check%20your%20mails. ';
        v_mobile:='716834441'; 
...
BEGIN
     ...
     SELECT HOST_NAME INTO GET_IP_OF_DATABASE FROM V$INSTANCE;
        ...
        IF IP_FLAG = 'Y' THEN
        CLARITY_ADMIN.PEND_MEGALINE_WO_DATA;
        END IF;
    ...
    FOR D_SUMMARY IN C_SUMMARY 
        LOOP
            ...
            || '</td><td>'
            || D_SUMMARY.MORE_THAN_7_DAYS
            || '</td><td>'
            || D_SUMMARY.TOTAL
            ||'</td></tr>';
        END LOOP;
        
    FOR D_MEGALINE_WO_COUNT IN C_MEGALINE_WO_COUNT 
        LOOP
            v_attr_table_html := v_attr_table_html
            || D_MEGALINE_WO_COUNT.REGION 
            || '</td><td>'
            || D_MEGALINE_WO_COUNT.PROVINCE
            || '</td><td>'
            || D_MEGALINE_WO_COUNT.RTOM
            || '</td><td>'
            -----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            ------|| D_MEGALINE_WO_COUNT.OWNER
           ---- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.NEW_CON_L
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.NEW_CON_G
            || '</td><td>'
            -----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.CREATE_OR_L
            || '</td><td>'
           ---- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.CREATE_OR_G
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.CREATE_RECON_L
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.CREATE_RECON_G
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_LOCATION_L
            || '</td><td>'
           ------ || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_LOCATION_G
            || '</td><td>'
            -----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_LOC_SAME_DP_L
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_LOC_SAME_DP_G
            || '</td><td>'
           ----- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_FEATURE_L
            || '</td><td>'
           ---- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_FEATURE_G
            || '</td><td>'
            -----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_FEATURE_OR_L
            || '</td><td>'
            ----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.MOD_FEATURE_OR_G
            || '</td><td bgcolor="#66FFFF">'
            || D_MEGALINE_WO_COUNT.NEW_CONN_SUB_TOTAL_L
            || '</td><td bgcolor="#66FFFF">'
            || D_MEGALINE_WO_COUNT.NEW_CONN_SUB_TOTAL_G
            || '</td><td>'
           ---- || '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'      
            || D_MEGALINE_WO_COUNT.REMOVAL_L
            || '</td><td>'
            ----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'      
            || D_MEGALINE_WO_COUNT.REMOVAL_G
            || '</td><td>'
            -----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.DELETE_OR_L
            || '</td><td>'
            ----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'
            || D_MEGALINE_WO_COUNT.DELETE_OR_G
            || '</td><td bgcolor="#CC99FF">'
            || D_MEGALINE_WO_COUNT.DEL_SUB_TOTAL_L
            || '</td><td bgcolor="#CC99FF">'
            || D_MEGALINE_WO_COUNT.DEL_SUB_TOTAL_G
            || '</td><td>'
            ----|| '</td><td'||GET_VAL(D_MEGALINE_WO_COUNT.OWNER)||'>'                 
            || D_MEGALINE_WO_COUNT.TOTAL
            || '</td></tr>';
        END LOOP;
 -- New Additions       
...
--  End of additon        
      ...
      qryCtx := DBMS_XMLGEN.newContext('SELECT * FROM CLARITY_ADMIN.PENDING_MEGALINE_WO_DETAILS');
      dest_lob := DBMS_XMLGEN.getXML(qryCtx);
      DBMS_XMLGEN.closeContext(qryCtx);
      begin
      l_body := '<html><head><title>MEGALINE Pending WO Details'
         || '</title></head><body>'
         || v_cl
         || v_cl
         ...
         || v_cl
         || '<br><br>'
         || 'Please find the Pending MEGALINE WO Detail Report as from 1st January 2014 to '
         || v_ren_date
         || 'Hrs.'
         || '<br>'
         || 'Report contain '
         ...
         || '<th COLSPAN="2">CREATE RECON</th>'
         || '<th COLSPAN="2">MOD LOCATION</th>'
         || '<th COLSPAN="2">MOD LOC SAME DP</th>' 
         || '<th COLSPAN="2">MOD FEATURE</th>'
         || '<th COLSPAN="2">MOD FEATURE OR</th>'
      ...
      send_mail(p_to          => 'chinwije@slt.com.lk',
                ...
                p_to_FR        => 'hemamalee@slt.com.lk',
                p_to_FS        => 'jayakody@slt.com.lk',
                p_to_FT        => 'lakshij@slt.com.lk',
                p_to_FU        => 'amilal@slt.com.lk',
                p_to_FV        => 'amilap@slt.com.lk',
                p_to_FW        => 'sampath@slt.com.lk',
                p_to_FX        => 'shyamalik@slt.com.lk',
                p_to_FY        => 'madhushasr@slt.com.lk',
                p_to_FZ        => 'sudayasiri@slt.com.lk',
                p_to_GA        => 'rtom@slt.com.lk',
                p_to_GB        => 'nihal@slt.com.lk',
                p_to_GC        => 'srnathan@slt.com.lk',
                p_from        => 'cdinesh@slt.com.lk',
                p_subject     => 'MEGALINE Pending WO Details',
                p_text_msg    => l_body,
                p_attach_name => 'MEGALINE_Pending_WO_Details.xls',
                p_attach_mime => 'xls',
