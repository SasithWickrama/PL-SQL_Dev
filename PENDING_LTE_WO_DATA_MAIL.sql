CREATE OR REPLACE PROCEDURE CLARITY_ADMIN.PENDING_LTE_WO_DATA_MAIL IS

    CURSOR C_DETAIL IS     
     SELECT count(*)
     FROM CLARITY_ADMIN.PENDING_LTE_WO;
    ...
    CURSOR C_LTE_WO_COUNT IS
     SELECT  
            ...
            COUNT(CASE SERVICE_TYPE WHEN 'AB-WIRELESS ACCESS' THEN 'X' ELSE NULL END) "AB_WIRELESS_ACCESS",
            COUNT(CASE SERVICE_TYPE WHEN 'BB-INTERNET' THEN 'X' ELSE NULL END) "BB_INTERNET",
            COUNT(CASE SERVICE_TYPE WHEN 'E-IPTV' THEN 'X' ELSE NULL END) "E_IPTV",
            COUNT(CASE SERVICE_TYPE WHEN 'V-VOICE' THEN 'X' ELSE NULL END) "V_VOICE",
            COUNT(*) "TOTAL"
            FROM  CLARITY_ADMIN.PENDING_LTE_WO
  ...
  l_name VARCHAR2(80) := 'LTE_Pending_WO_Details.xls';
  l_blob CLOB;
...
dbms_output.put_line('@6') ;
 ...
 dbms_output.put_line('@7') ;
  ...
  v_sms_txt := 'Dear%20All%20LTE%20Pending%20WO%20detail%20report%20as%20@%20today%20contain%20' ||v_ren_CNT|| '%20records.%20Please%20check%20your%20mails. ';
        v_mobile:='0716834441'; 
...
BEGIN
       ...
       SELECT HOST_NAME INTO GET_IP_OF_DATABASE FROM V$INSTANCE;
        ...
        IF IP_FLAG = 'Y' THEN
        CLARITY_ADMIN.PENDING_LTE_WO_DATA;
        END IF;
    ...
    IF v_ren_CNT >=0 THEN
      ...
      FOR D_LTE_WO_COUNT IN C_LTE_WO_COUNT 
        LOOP
            v_attr_table_html := v_attr_table_html 
            || D_LTE_WO_COUNT.REGION 
            || '</td><td>'
            || D_LTE_WO_COUNT.PROVINCE 
            || '</td><td>'
            || D_LTE_WO_COUNT.RTOM
            || '</td><td>'
            || D_LTE_WO_COUNT.AB_WIRELESS_ACCESS
            || '</td><td>'
            || D_LTE_WO_COUNT.BB_INTERNET
            || '</td><td>'
            || D_LTE_WO_COUNT.E_IPTV
            || '</td><td>'
            || D_LTE_WO_COUNT.V_VOICE
            || '</td><td>'
            || D_LTE_WO_COUNT.TOTAL 
            ||'</td></tr>';
        END LOOP;
          
      qryCtx := DBMS_XMLGEN.newContext('SELECT * FROM CLARITY_ADMIN.PENDING_LTE_WO');
      dest_lob := DBMS_XMLGEN.getXML(qryCtx);
      DBMS_XMLGEN.closeContext(qryCtx);
      begin
      l_body := '<html><head><title>LTE Pending WO Details'
         || '</title></head><body>'
         || v_cl
         || v_cl
         ...
         || v_cl
         || '<br><br>'
         || 'Please find the LTE Pending Work Order Detail Report AS @ '
         || '<br>'
         || v_ren_date
         || '.'
         || '<br>'
      ...
      send_mail(p_to          => 'cdinesh@slt.com.lk',
                p_to_A        => 'anuruddha@slt.com.lk',
                p_to_B        => 'vijayantha@slt.com.lk',
                p_to_C        => 'rtom@slt.com.lk',
                p_to_D        => 'nihal@slt.com.lk',
                p_to_E        => 'srnathan@slt.com.lk',
             /*   p_to_F        => 'priyanjana@slt.com.lk',
                ...
                p_subject     => 'LTE Pending WO Details',
                p_text_msg    => l_body,
                p_attach_name => 'LTE_Pending_WO_Details.xls',
                p_attach_mime => 'xls',
 ...
 DELETE FROM CLARITY_ADMIN.PENDING_LTE_WO;
