CREATE OR REPLACE PROCEDURE CLARITY_ADMIN.SEND_MAIL IS
...
BEGIN
    ...
    IF v_ren_date IS NOT NULL THEN
    
      begin
      l_body := '<html><head><title>TEST Mail'
         || '</title></head><body>'
         || v_cl
         || v_cl
         ...
         || v_cl
         || '<br><br>'
         || 'Please find the mail Detail Report AS @ '
         || v_ren_date
         || '.'
         || '<br>'
         || 'Report contain '
         ...
         || v_cl
         || '<br>Dinesh Perera.'
         || '<br>Engineer - OSS System Development & Operations'
