CREATE OR REPLACE PROCEDURE CLARITY_ADMIN.PEND_KY_MDF_WO_DETAILS_MAIL IS

    CURSOR C_DETAIL IS     
     SELECT count(*)
     FROM CLARITY_ADMIN.PENDINGE_KY_MDF_WO_DETAILS;
  ...
  l_name VARCHAR2(80) := 'CP_MDF_Pending_WO_Details.xls';
  l_blob CLOB;
...
dbms_output.put_line('@6') ;
 ...
 dbms_output.put_line('@7') ;
  ...
  v_sms_txt := 'Dear%20All%20Central%20Province%20MDF%20pending%20WO%20detail%20report%20as%20@%20today%20contain%20' ||v_ren_CNT|| '%20records.%20Please%20check%20your%20mails. ';
        v_mobile:='0715360901'; 
...
BEGIN
      ...
      SELECT HOST_NAME INTO GET_IP_OF_DATABASE FROM V$INSTANCE;
        ...
        IF IP_FLAG = 'Y' THEN  
        CLARITY_ADMIN.PEND_KY_MDF_WO_DETAILS_DATA;
        END IF;
    ...
    FOR D_KY_SW_COUNT IN C_KY_WO_COUNT 
      ...
      qryCtx := DBMS_XMLGEN.newContext('SELECT * FROM CLARITY_ADMIN.PENDINGE_KY_MDF_WO_DETAILS');
      dest_lob := DBMS_XMLGEN.getXML(qryCtx);
      DBMS_XMLGEN.closeContext(qryCtx);
      begin
      l_body := '<html><head><title>CP MDF Pending WO Details'
         || '</title></head><body>'
         || v_cl
         || v_cl
         ...
         || v_cl
         || '<br><br>'
         || 'Please find the Central Province MDF Pending WO Detail Report AS @ '
         || v_ren_date
         || '.'
         || '<br>'
         || 'Report contain '
      ...
      send_mail(p_to          => 'subhashini@slt.com.lk',
               ...
               /* p_to_B        => 'cdinesh@slt.com.lk',
                ...
                p_subject     => 'CP MDF Pending WO Details',
                p_text_msg    => l_body,
                p_attach_name => 'CP_MDF_Pending_WO_Details.xls',
                p_attach_mime => 'xls',
