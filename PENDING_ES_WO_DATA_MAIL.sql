CREATE OR REPLACE PROCEDURE CLARITY_ADMIN.PENDING_ES_WO_DATA_MAIL IS

    CURSOR C_DETAIL IS     
     SELECT count(*)
     FROM CLARITY_ADMIN.PENDING_ES_WO;
    ...
    CURSOR C_ES_WO_COUNT IS
     SELECT  
            ...
            SUM(CASE WHEN DURATION = 0 THEN 1 ELSE 0 END )AS "ZERO",
            SUM(CASE WHEN DURATION = 1 THEN 1 ELSE 0 END )AS "ONE",
            SUM(CASE WHEN DURATION = 2 THEN 1 ELSE 0 END )AS "TWO",
            SUM(CASE WHEN DURATION = 3 THEN 1 ELSE 0 END )AS "THREE",
            SUM(CASE WHEN DURATION = 4 THEN 1 ELSE 0 END )AS "FOUR",
            SUM(CASE WHEN DURATION = 5 THEN 1 ELSE 0 END )AS "FIVE",
            SUM(CASE WHEN DURATION = 6 THEN 1 ELSE 0 END )AS "SIX",
            SUM(CASE WHEN DURATION = 7 THEN 1 ELSE 0 END )AS "SEVEN",
            SUM(CASE WHEN DURATION > 7 THEN 1 ELSE 0 END )AS "MORE_THAN_SEVEN",
            COUNT(*) "TOTAL"
            FROM  CLARITY_ADMIN.PENDING_ES_WO
         ...
         CURSOR CLA_PRESENTAGES  IS 
            ...
            '<td>'||ROUND(((ZERO / TOTAL) *100),0)||'%</td>'||
            '<td>'||ROUND(((ONE / TOTAL) *100),0)||'%</td>'||
            '<td>'||ROUND(((TWO  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((THREE  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((FOUR  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((FIVE  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((SIX  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((SEVEN  / TOTAL) *100),0) ||'%</td>'||
            '<td>'||ROUND(((MORE_THAN_SEVEN  / TOTAL) *100),0) ||'%</td>'||
  ...
  l_name VARCHAR2(80) := 'Enterprise_Pending_WO_Details.xls';
  l_blob CLOB;
  l_body            VARCHAR2(14000);
  v_attr_table_html varchar2(12000);  ---  4000
...
/*  UTL_SMTP.write_data(l_mail_conn, 'To: ' || p_to_A || UTL_TCP.crlf);
  ...
  UTL_SMTP.write_data(l_mail_conn, 'Bcc: ' || p_to_BN || UTL_TCP.crlf);
...
dbms_output.put_line('@6') ;
 ...
 dbms_output.put_line('@7') ;
  ...
  v_sms_txt := 'Dear%20All%20Enterprise%20Pending%20WO%20detail%20report%20as%20@%20'||v_ren_date||'%20contain%20' ||v_ren_CNT|| '%20records.%20Please%20check%20your%20mails.';
       /* v_mobile:='0714102000'; 
...
BEGIN
       ...
       SELECT HOST_NAME INTO GET_IP_OF_DATABASE FROM V$INSTANCE;
        ...
        IF IP_FLAG = 'Y' THEN
        CLARITY_ADMIN.PENDING_ES_WO_DATA;
        END IF;
    ...
    IF v_ren_CNT >=0 THEN
      ...
      FOR D_ES_WO_COUNT IN C_ES_WO_COUNT 
        LOOP
            v_attr_table_html := v_attr_table_html 
            || D_ES_WO_COUNT.REGION 
            || '</td><td>'
            || D_ES_WO_COUNT.PROVINCE 
            || '</td><td>'
            || D_ES_WO_COUNT.RTOM
            || '</td><td>'
            || D_ES_WO_COUNT.ZERO
      ...
      qryCtx := DBMS_XMLGEN.newContext('SELECT * FROM CLARITY_ADMIN.PENDING_ES_WO');
      dest_lob := DBMS_XMLGEN.getXML(qryCtx);
      DBMS_XMLGEN.closeContext(qryCtx);
      begin
      l_body := '<html><head><title>Pending Enterprise WO Details'
         || '</title></head><body>'
         || v_cl
         || v_cl
         ...
         || v_cl
         || '<br><br>'
         || 'Please find the Pending Enterprise Work Order Detail Report AS @ '
         || v_ren_date
         || 'Hrs.'
         || '<br>'
         || 'Report contain '
      ...
      send_mail(p_to          => 'jagodage@slt.com.lk',    ----  chinwije
              /*  p_to_A        => 'manjula@slt.com.lk',
                ...
                p_to_BN       => 'cdinesh@slt.com.lk',
                p_from        => 'cdinesh@slt.com.lk',
                p_subject     => 'Enterprise Pending WO Details',
                p_text_msg    => l_body,
                p_attach_name => 'Enterprise_Pending_WO_Details.xls',
                p_attach_mime => 'xls',
